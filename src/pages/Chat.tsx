...
export function Chat() {
  ...
  const upload = search.upload as string | undefined;
  const child = search.child as string | undefined;
  ...
  async function send(text: string) {
    ...
    setMsgs(m => [...m.map(x => ({ ...x, chips: false })),
    ...
    try {
      ...
      if (!convId) {
        ...
        setTimeout(() => {
          sb.from("conversations").select("title").eq("id", data.conversation_id).single()
